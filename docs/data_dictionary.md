...
一行一个由规范化站名生成的候选站点键。保留规范名称、规范化名称、源 ID 数量、别名数量、有效时间、端点记录数和映射审计状态。

### `station_alias`

一行一个候选站点键、源站点 ID 和原始站名组合。`requires_review` 标记同一源 ID 是否指向多个候选站点键。

### `fact_od_flow`

一行一个候选起点和候选终点组合。包含同站标志、行程次数、时长总和、平均时长及 `1-3分钟`、`3-15分钟`、`15分钟以上` 三个时长分层。

### `fact_station_period`

一行一个站点、日期类型和小时组合。包含流出、流入和 `净流入=流入-流出`。

### `fact_same_station`

一行一个站点、日期类型和小时组合，只统计同站归还及其时长分层。
